document.addEventListener('DOMContentLoaded', function() {
        ...
        const getAllWidth =
            ele => {
              return Array.from(ele).reduce(
                  (width, i) => width + i.offsetWidth, 0)
            }

        if (init) {
          const blogInfoWidth =
              getAllWidth(document.querySelector('#blog-info > a').children)
          const menusWidth =
              getAllWidth(document.getElementById('menus').children)
  ...
  const scrollDownInIndex = () => {
    const handleScrollToDest =
        () => {
          btf.scrollToDest(
              document.getElementById('content-inner').offsetTop, 300)
        }
  ...
  const addHighlightTool = () => {
    ...
    const $figureHighlight = plugin === 'highlight.js' ?
        document.querySelectorAll('figure.highlight') :
        document.querySelectorAll('pre[class*="language-"]')

    if (!((isShowTool || highlightHeightLimit) &&
          $figureHighlight.length)) return
    ...
    const highlightShrinkFn =
        ele => {
          ele.classList.toggle('closed')
        }

    const highlightToolsFn =
        function(e) {
  ...
  const runJustifiedGallery = (item, data, isButton = false, tabs) => {
    ...
    else {ig.on('requestAppend', requestAppendFn)
      ig.renderItems()
    }
  ...
  const addJustifiedGallery = async (ele, tabs = false) => {
    const init =
      ...
      for (const item of ele) {
        ...
        const content = item.getAttribute('data-type') === 'url' ?
            await fetchUrl(text) :
            JSON.parse(text)
  ...
  */
  const scrollFnToDo = () => {
    ...
    if (!($article && (isToc || isAnchor))) return
        ...
        let $tocLink,
        $cardToc, autoScrollToc, $tocPercentage, isExpand
      ...
      const tocItemClickFn =
          e => {
            ...
            if (!target) return
                ...
                e.preventDefault()
            btf.scrollToDest(
                btf.getEleTop(document.getElementById(
    ...
    const findHeadPosition =
        top => {
          ...
          if (isToc) {
            $cardToc.querySelectorAll('.active').forEach(
  ...
  document.getElementById('rightside').addEventListener('click', function (e) {
    ...
    if ($target && rightSideFn[$target.id]) {
      ...
      const clickFnOfSubMenu =
          () => {
            const handleClickOfSubMenu =
                e => {
                  const target = e.target.closest('.site-page.group')
                  if (!target) return target.classList.toggle('hide')
                }
      ...
      const tabsFn =
          () => {
            const navTabsElement =
                document.querySelectorAll('#article-container .tabs')
    ...
    window.addEventListener('resize', () => {
      ...
      document.getElementById('menu-mask')
          .addEventListener('click', e => {sidebarFn.close()})
        ...
        window.matchMedia('(prefers-color-scheme: dark)')
            .addEventListener('change', e => {
              if (saveToLocal.get('theme') !== undefined)
                return e.matches ? handleThemeChange('dark') :
